...
class GetCompaniesExtendedV2HandlerTest extends TestCase
{
    public function testHandle()
    {
        ...
        /** @var CompanyExtendedV2ResultData $response */
        $response = $handler->handle(new GetCompaniesExtendedV2());

        $this->assertInstanceOf(CompanyExtendedV2ResultData::class, $response);
        $this->assertContainsOnlyInstancesOf(CompanyExtendedV2::class, $response->items);
        $this->assertContainsOnlyInstancesOf(CompanyProfileV2Address::class, $response->items[0]->addresses);
        $this->assertInstanceOf(CompanyProfileV2TradeNames::class, $response->items[0]->tradeNames);
        $this->assertContainsOnlyInstancesOf(CompanyProfileV2BusinessActivity::class, $response->items[0]->businessActivities);
